    ...
    <table class="w-full text-sm">
        ...
        @foreach($collegiUninominali as $collegio)
            <tr class="tr-standard">
                <td><a href="{{ route('collegio_uninominale_senato', $collegio) }}" class="anchor">{{ $collegio->nome }}</a></td>
            </tr>
        @endforeach
    </table>
    ...
    <table class="w-full text-sm">
        ...
        @foreach($candidature as $candidatura)
            ...
            @foreach($candidatura->candidati as $candidato)
                <tr class="tr-standard">
                    <td><a href="{{ route('collegio_uninominale_senato', $collegio) }}" class="anchor">{{ $candidato->cognome }} {{ $candidato->nome }} {{ $candidato->altro_1 }} {{ $candidato->altro_2 }}</a></td>
