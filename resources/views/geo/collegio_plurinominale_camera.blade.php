...
@section('content')
    ...
    <div class="my-2 text-sm">
        ...
        <a href="{{ route('circoscrizione_camera', $collegio->circoscrizione) }}" class="anchor">{{ $collegio->circoscrizione->nome }}</a> ~ 
    ...
    <table class="w-full text-sm">
        ...
        @foreach($collegiUninominali as $collegio)
            <tr class="even:bg-slate-100 odd:bg-slate-200">
                <td class="p-1"><a href="{{ route('collegio_uninominale_camera', $collegio) }}" class="anchor">{{ $collegio->nome }}</a></td>
            </tr>
        @endforeach
    </table>
    ...
    <table class="w-full text-sm">
        ...
        @foreach($candidature as $candidatura)
            ...
            @foreach($candidatura->candidati as $candidato)
                <tr class="even:bg-slate-100 odd:bg-slate-200">
                    <td class="p-1"><a href="{{ route('collegio_uninominale_camera', $collegio) }}" class="anchor">{{ $candidato->cognome }} {{ $candidato->nome }} {{ $candidato->altro_1 }} {{ $candidato->altro_2 }}</a></td>
