    ...
    <table class="table">
        ...
        @foreach($collegiUninominali as $collegio)
            <tr class="tr-standard">
                <td><a href="{{ route('collegio_uninominale_camera', $collegio) }}" class="anchor">@svg('heroicon-o-cursor-arrow-rays', 'w-5 h-5 inline-block') {{ $collegio->nome }}</a></td>
            </tr>
        @endforeach
    </table>
    ...
    <table class="table">
        ...
        @foreach($candidature as $candidatura)
            ...
            @foreach($candidatura->candidati as $candidato)
                <tr class="tr-standard">
                    <td><a href="{{ route('collegio_uninominale_camera', $collegio) }}" class="anchor">@svg('heroicon-o-user-circle', 'w-5 h-5 inline-block') {{ $candidato->cognome }} {{ $candidato->nome }} {{ $candidato->altro_1 }} {{ $candidato->altro_2 }}</a></td>
